...
## 🧹 マージ済みブランチの削除は定期作業（毎回やらない）

マージのたびに毎回消す必要はない。マージ済みブランチが**溜まってきたら**まとめて掃除する。
`git branch --merged main` で確認 → `git branch -d`（＋ push 済みなら `git push origin --delete`）→ `git remote prune origin`。
worktree 使用中 (`+` 印) のブランチや、未マージ (`git branch --no-merged main` に出る) ブランチは消さない。
詳細は `AI_HANDOFF.md` の「マージ済みブランチの削除は『定期作業』」を参照。
