...
- `schedule_id` は新規マスタ作成時に `computeTentativeScheduleId_()` で `APP_DATA.schedule` の最大値+1 を `SCH_xxx` 形式で仮表示する（disabled=サーバー採番は不変）。`pr_id` も同様に `computeTentativePrId_()` で仮番号表示。`collectNamedFormValues` は disabled を skip するためペイロードには含まれない (1.30)。
- コメント本文の URL (`https?://...`) は `linkifyText_` で `<a target="_blank" rel="noopener">` に変換して表示する。escapeHtml → URL正規表現置換の順でXSS対策済み (`ClientComments.html` 1.30)。
...
- **設定者(assignee)が `R` のとき editModal の「設定」ボタン (`#btnModalSetting`) は非活性。「確認」のみ押下可**。`applyRSetterButtonLock_(isR)` が制御し、`syncReviewerLock_('editModal')`(設定者select変更時)と `initEditModalCheckButtons_()`(モーダル展開直後)の双方から呼ばれる。R判定はフォーム現在値で行うため、設定者をRに変えた瞬間も追従する (1.26)。
