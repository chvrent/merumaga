...
- **設定者(assignee)が `R` のとき editModal の「設定」ボタン (`#btnModalSetting`) は非活性。「確認」のみ押下可**。`applyRSetterButtonLock_(isR)` が制御し、`syncReviewerLock_('editModal')`(設定者select変更時)と `initEditModalCheckButtons_()`(モーダル展開直後)の双方から呼ばれる。R判定は select 値に加え `CURRENT_ENTRY` の `getValueByKeys(['assignee','設定者','setter'])` も OR で判定（`refreshAdminSelects_` 経由の遅延呼び出しで select options 再構築中に値が空になる場合でも正しく R 検出する）。`handleModalToggleSetting_` 冒頭にも R ガードあり。`.modal-footer-btn:disabled` CSS で disabled 時は `opacity:0.4` + `pointer-events:none` (1.26, 1.36)。**教訓: 非同期初期化(`requestAnimationFrame`→`finalizeEditModalContent_`)で後から呼ばれる関数が先の disabled 設定を上書きする「後勝ち」パターンに注意。最後に呼ばれる関数でも正しく判定されるよう、複数ソース(select値+エントリデータ)で OR 判定する**。
