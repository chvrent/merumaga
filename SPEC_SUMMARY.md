...
- 追従させないための要点: `.master-modal-card` は flex 列なので `#masterModal .form-grid{overflow-y:visible;flex-shrink:0}` でフォームの内部スクロールを止め、**カード全体を単一スクロール領域**にする (`Styles.html`)。これをしないと flex でフォームだけが内部スクロールし、保存/キャンセル・削除が下部に張り付く (追従フッター化) (1.28)。
