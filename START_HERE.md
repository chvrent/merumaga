...
## Calendar Color Rules (2026-05-25 改訂)

| Category | Type | Color | Description |
| :--- | :--- | :--- | :--- |
| 検証中・サイクル:単発 | 背景色 | `#d9e6fc` | 「検証中」チェックあり、またはサイクルが「単発」の場合 |
| 担当部署: ウキ課以外 | 背景色 | `#fce5cd` | 担当部署名に「ウキ課」が含まれない場合 |
| サイクル:月末配信 | 左ライン | `#fff2cc` | サイクルが「毎月配信」または「月末増発」の場合 |
| 隔週配信A | 左ライン | `#b7e1cd` | サイクルが「隔週配信A」の場合。バッジも連動。 |
| 隔週配信B | 左ライン | `#ffe1f6` | サイクルが「隔週配信B」の場合。バッジも連動。 |
| 特殊サイクル | 左ライン | `#d9d2e9` | サイクルが「特殊サイクル1-3」の場合。バッジも連動。 |
| MA | 背景色 | `#FFF8D6` | 種別が「MA」の場合 |
| 担当部署: 商品 | 左ライン | `#008080` | 担当部署名に「商品」を含む場合 |
