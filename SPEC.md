...
- 主データソース: `app_schedule`, `app_pr`, `app_pr_targets`, `app_product`, `app_comments`, `app_schedule_archives`, `app_exceptions`, `app_check_status`
...
求人件数は `job_url` から `current_job_count` と `job_count_updated_at` を更新し、毎週火曜4時のトリガーで最新化する。保存時の即時更新も残してあるため、編集直後の表示と定期更新の両方で値が揃う。設定・確認の赤塗り状態は `app_check_status` に保存され、ブラウザを閉じても復元される。
