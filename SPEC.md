...
| 2026-05-25 | 1.6 | カレンダー色分けルールの刷新（背景と左ラインの併用）、検証中（is_verifying）項目の追加、およびデプロイ前強制確認ルールの追加。 | Gemini CLI |
...
- **新規（`is_new`） / 検証中（`is_verifying`）**: **【重要】必ずチェックボックス（`<input type="checkbox">`）として実装。** テキストエリアやテキストボックスは禁止。ラベルは「新規」「検証中」。内部キー名 `is_new` / `is_verifying` とし、スプレッドシートの該当列と連動させる。
...
- **配信編集モーダル (#editModal)**:
    ...
    - **【配信編集専用ロック】**: 配信実績の整合性を保つため、実装定数で指定された項目のみ編集不可（グレーアウト）とする。
...
- **下書き保存**: 新規追加時は「下書き保存」を選べる。下書きは `is_draft` 列で管理し、メルマガ・PR一覧には表示するが、配信カレンダーやPR表示判定には反映しない。通常の保存時は下書き状態を解除する。
...
#### ［フィルタ項目マトリクス］
状態、サブカテゴリ、形式、種別は `<select>` 要素（チップ）として、新規のみは `<button>` 要素（アクティブ時に青背景）として実装する。

| フィルタ項目名 | UIの型 | 選択肢 | 初期値 | ロジック（GAS/JS連動） |
| --- | --- | --- | --- | --- |
| **状態** | `select` | 配信中 / 終了 | 配信中 | 終了日（`end_date`）が当日を過ぎていないものを「配信中」とする。 |
| **新規のみ** | `button` | (トグル式) | OFF | **【重要】**有効時、`is_new` 列が 「TRUE」のレコードだけを抽出。 |
...
| **種別 (`mail_type`)** | `select` | 🟢可能 | 🟢可能 | 🟢可能 | マスタ編集・配信編集とも編集可能。 |
| **サイクル** | `select` | 🟢可能 | 🟢可能 | 🟢可能 | マスタ編集・配信編集とも編集可能。 |
...
### 4.4 PRラベル判定と優先順位
- **紐付け方式の優先順位**:
    1. **新方式 (ID指定)**: `app_pr.target_ids` にメルマガの `schedule_id` が含まれている場合。
    2. **準新方式 (名前一致)**: `app_pr_targets.mail_name` がメルマガ名と一致する場合。
    3. **旧方式 (直書き)**: メルマガマスタの `pr` 列に PR ID が直接入力されている場合。
...
- **PRマスタ (`app_pr`)**: `PR[` + 連番 + `]` (例: `PR[1]`)。
- **採番ロジック**: 該当シートの全行を走査し、現在の最大値 + 1 を採用。
...
2. **初期選択と必須バリデーション（required）**
   ...
   - HTML의 `<select>` タグ自体に `required` 属性を付与し、ユーザーがここを切り替えない限り、フォームの「保存」ボタンを押してもブラウザ側で自動的に送信がブロックされるようにする。
...
| カテゴリ | 表示形式 | 色 | 適用条件 |
| :--- | :--- | :--- | :--- |
| **検証中・単発** | 背景色 | `#d9e6fc` | 「検証中」チェックあり、またはサイクル「単発」 |
| **担当部署:ウキ課以外**| 背景色 | `#fce5cd` | 担当部署名に「ウキ課」が含まれない場合 |
| **サイクル:月末配信** | 左ライン | `#fff2cc` | サイクル「毎月配信」「月末増発」 |
| **隔週配信A** | 左ライン | `#b7e1cd` | サイクル「隔週配信A」。ヘッダーバッジも連動。 |
| **隔週配信B** | 左ライン | `#ffe1f6` | サイクル「隔週配信B」。ヘッダーバッジも連動。 |
| **特殊サイクル** | 左ライン | `#d9d2e9` | サイクル「特殊サイクル1-3」。ヘッダーバッジも連動。 |
| **MA** | 背景色 | `#FFF8D6` | 種別「MA」 |
