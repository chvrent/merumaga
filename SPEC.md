...
| 日付 | 版 | 概要 | 担当者 |
| :--- | :--- | :--- | :--- |
| 2026-05-31 | 1.52 | **`is_new` を初回配信日フィールドへ変更** — `is_new` はチェックボックスではなく `YYYY-MM-DD` の初回配信日として扱う。マスタ新規追加時、同名メルマガが `app_schedule` 全履歴に無い場合のみ初回発生日を自動入力し、モーダル内の新規欄はロック表示。メルマガ一覧の「新規」チップは年月ドロップダウンで `is_new` の年月に一致する行を絞り込み、カレンダーセルには該当日に「新規」バッジ、上部には月次新規サマリを表示。旧 `TRUE` 値は読み取り時に初回発生日へフォールバックし、移行用 `migrateIsNewToFirstDeliveryDates()` を追加。 | Codex |
...
| 2026-05-30 | 1.44 | **UI刷新バッチ2（配信編集/マスタ）** — ①ヘッダーの通数/設定/確認プルダウンを半幅化。②フッターに「設定済→確認待ち→確定済」の3段ステータス進行を常時表示。③備考内URLを欄外リンクボタンとして表示。④配信編集のセクション分けをマスタ新規/編集にも共通適用（`SECTION_MAP_`/`SECTION_ORDER_` を共有化）。⑤新セクション「自動求人特集」を追加し該当項目をまとめる。PR新規追加のメルマガ選択エリアを新デザイン化、PR本文をフル幅に。⑥PR本文は全文表示＋クリックでコピー、今週PR追加/削除が必要なメルマガを配信編集モーダル上部に全件アラート表示（`getWeekPrActions_`）。⑦メルマガ内容・備考の入力欄を2倍縦幅、パラメータを1行入力に。 | Claude Opus |
| 2026-05-30 | 1.43 | **UI刷新バッチ（一覧/モーダル/カレンダー）** — ①配信編集の設定/確認は略称のみ表示・プルダウン拡大・パラメータを1行入力に。③凡例ラベル簡潔化。④メルマガ一覧/PR管理を画面いっぱい表示・列ヘッダー中央寄せ。⑤PR新規追加のメルマガ選択エリアを新デザイン化・PR本文をフル幅に。⑥マスタ編集の「コピーして作成」を下部フッター左隅へ移動。⑦日付入力はクリックで即ネイティブカレンダー(`showPicker`)。⑧カレンダー日付ヘッダーを整理（日付→サイクル/月末タグ→操作ボタン）、設定/確認のアクティブ赤(`is-active-red`)を**白文字×濃背景**に変更（occurrence override の赤と区別）。マスタ変更履歴(②)は従来どおり既存マスタ編集で表示。 | Claude Opus |
...
> 特に「色トークン」「`is-active-red` の配色」「データ可視化色」「ロック対象(is_new/is_verifying)」「削除済み要素」を元に戻さないこと。

### 0.1 デザイン言語（正本: `Styles.html`）
- フォント: **Inter**（@import）。基準 `font-size:13px`。ただしカレンダーの密度維持のため `.mail-name`/通数/設定/確認列は **11px 固定**。
- 配色トークン（`:root`）: bg `#ffffff` / bg-sub `#f8f9fa` / bg-hover `#f0f2f5` / border `#e2e5e9` / border-strong `#cbd0d8` / text `#0d1117` / text-2 `#424a55` / text-3 `#7a8490`。
- **teal アクセント**: `--accent #0d9488` / `--accent-dark #0f766e` / `--accent-light #ccfbf1` / `--accent-mid #99f6e4`。alert `#dc2626` / warn `#d97706` / ok `#16a34a`。角丸 `--r8 6px` / `--r4 4px`。
- トップバー（ナビ）は**ダーク背景 `#0d1117`**、アクティブタブは **teal 下線**。主要CTA（新規追加/一括保存相当/保存/送信/モーダル primary）と各種アクティブ状態は **teal**。
- バッジ/チップは**ピル型**。設定(setter/state-btn)=**インディゴ `#4338ca/#eef2ff/#c7d2fe`**、確認(checker/checking)=**緑(ok系)**。設定者インディゴ・確認者緑は維持。
- 絵文字は **Tabler アイコン**に統一（コピー=`ti-copy`、閉じる=`ti-x`、保存=`ti-device-floppy`、停止=`ti-player-pause`、再開=`ti-player-play`、お知らせ=`ti-speakerphone`）。
- `--pink` 系は廃止（変数定義のみ残置・未使用）。新たに pink を使わない。

### 0.2 データ可視化色（正本: `Client.html` 定数。`Styles.html` は追従）
- `SCHEDULE_LINE_COLORS`: 隔週A `#34d399` / 隔週B `#ec4899` / 特殊 `#8b5cf6`。
- `SCHEDULE_BACKGROUND_COLORS`: 検証中・単発・新規 `#dbeafe` / 月末配信 `#ede9fe` / 商品 `#dcfce7`。`Styles` の `--color-month-end-bg` は monthEnd と**一致必須**。
- **設定/確認セルのアクティブ表示 `.is-active-red` = 白文字 × `--accent`（#0d9488）**。occurrence override（変更あり）の赤と区別する目的。**赤に戻さない。**
- occurrence override（変更あり）= **赤(alert)**。
- 凡例ラベル: `検証中・単発・新規` / `月末配信` / `商品` / `隔週A` / `隔週B` / `特殊サイクル`。

### 0.3 カレンダー
- 旧「注記バー（差分保護テキスト）」は**撤去** → **PRアナウンスバー**（`#prAnnounceBar`, 表示中の週に掲出中の PR を表示）。
- 「本日の状態を一括保存」ボタンは**廃止**（設定/確認はクリックでトグルしない）。
- 設定/確認セルは**ボタン感なし**。**行全体クリック**で配信編集モーダルを開き、同時に**メルマガ名をコピー**。
- 検索は2系統: **メルマガ名**（部分一致）＋**担当者**（`app_admin_master` 連動プルダウン・**完全一致**・**設定者or確認者**に一致で対象・未設定は除外）。アーカイブ枠も同条件で絞り込む。
...
- フィルター行は PR管理／メルマガ一覧で**縦幅・余白を統一**（`.filter-chips-bar` 40px、内側 `.pr-status-chips` の二重 padding を打ち消し）。タブ遷移でガタつかせない。
...
- フッター: **3段ステータス進行（設定済→確認待ち→確定済）を常時表示**。確定済は `APP_DATA.fixedOccurrences` 由来。
- 変更範囲選択: 「この日だけ変更」「●〜まで」「**この日以降を変更（マスタ編集）**」(`saveScheduleFromDate`。**確定済・確認済みではない同じメルマガ予定がすべて変更される**旨のアラート併記)。
- PR行: 本文は**全文表示＋クリックでコピー**。掲出中= teal。今週このメルマガに PR追加/削除がある場合は上部に「PRのお知らせ」。
- **マスタ変更後の初回配信**は「変更あり」バナー（`#editChangeBanner`）＋最下部にマスタ変更履歴（`#editMasterChangeHistory`）を表示。
- 備考URLは**欄外リンクボタン**。メルマガ内容・備考の入力欄は**2倍縦幅(144px)**。パラメータは**1行入力＋横幅いっぱい**。
- **新規(is_new)/検証(is_verifying) は配信編集モーダルでロック**（マスタ値に紐づけ・占有編集不可）。変更は**マスタ編集モーダル**で行う（START_HERE / SPEC 1.6 準拠）。
- ロック表示は text-2 / bg-hover / border-strong ＋鍵アイコン（濃く読みやすく）。

### 0.6 マスタ新規/編集モーダル
- セクション分け（基本設定 / メルマガ内容 / 自動求人特集 / ステータス / 抽出内容）を配信編集と**共通**（`SECTION_MAP_`/`SECTION_ORDER_`/`FULLWIDTH_KEYS_`）。
- **自動求人特集**セクション = `job_url` / `auto_job_feature_id` / `auto_job_other_condition` / `current_job_count`(自動求人特集_求人数)。
...
- **JOB_/USER_ 項目のマスタ紐づけ複数選択（実装済み・版1.48）**:
  ...
  - **チップ表示の集約**: 大分類が**全選択**のときは小分類を並べず**大分類1個の集約チップ（`.ms-chip-group`・濃色）**で表示する（初期は `buildMsChipsFromGroups_()`、変更時は `collectMsChipsFromDom_()`）。「その他（マスタ外）」グループは `collapsible=false` で常に個別表示。
  - **大分類＝小分類で1個だけ**のグループ（`group.label === items[0]` かつ `items.length===1`）は `collapsible=false` とし、**重複する大分類トグルを描画せず選択肢1個のみ**を表示する。
  - **年齢（`target_age`）はマスタ非依存の埋め込み複数選択**（`EMBEDDED_MULTISELECT_OPTIONS_` / `AGE_MULTISELECT_OPTIONS_`）。単一グループ「全て選択する」を大分類トグルとして全選択／全集約に利用する。選択肢=`19歳以下 / 19～25 / 26～29 / 30～35 / 36～39 / 40～45 / 46～49 / 50～55 / 56～59 / 60～65 / 66～69 / 70～75 / 76～79 / 80～85 / 86～89 / 90～95 / 96～99`。
