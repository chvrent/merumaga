...
- 通常枠の `設定` は画面上のメモ扱いで、GAS保存対象にしない。
- 一括保存時は、通常枠は `確認` が有効な行、R枠は `設定` が有効な行だけを確定差分として `saveDailyArchiveDiffs()` に渡す。
- 日別ヘッダーの `R一括設定` は、その日付列のR枠の `設定` だけをローカルで赤塗りする。GAS通信は行わない。
