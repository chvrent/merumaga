...
| 2026-05-30 | 1.36 | **配信編集モーダルの「設定」ボタンが反応しなくなるバグを修正** — `applyRSetterButtonLock_` が `isR=false` 時に `btnModalSetting.disabled = false` を設定していなかった。一度 設定者R のエントリを開くと `disabled=true` がそのまま残り、以降の全エントリで設定ボタンが押せなくなっていた。`else` 分岐で `disabled=false` を追加 (`ClientModals.html`)。staging @273。 | Claude Opus |
